...
function wpacptdcf7_products_shortcode_handler( $tag ) {
    ...
    if(!empty($tag->get_option( 'category' )[0]) && empty($tag->get_option( 'featured' )[0]) && empty($tag->get_option( 'tags' )[0]) && empty($tag->get_option( 'bestselling' )[0])) {
    ...
    }else if(!empty($tag->get_option( 'featured' )[0]) && empty($tag->get_option( 'tags' )[0]) && empty($tag->get_option( 'category' )[0]) && empty($tag->get_option( 'bestselling' )[0])) {
        $woo_product_posts = get_posts( array(
        'post_type' => 'product',
        'post_status' => 'publish',
        'numberposts' => -1,
    ...
    }else if(!empty($tag->get_option( 'tags' )[0]) && empty($tag->get_option( 'category' )[0]) && empty($tag->get_option( 'featured' )[0]) && empty($tag->get_option( 'bestselling' )[0])) {
        $woo_product_posts = get_posts( array(
        'post_type' => 'product',
        'post_status' => 'publish',
        'numberposts' => -1,
            'tax_query'      => array(
                array(
                    ...
                    'field'    => 'name', 
                    'terms'    => array( $tag->get_option( 'tags' )[0] ) 
                )
            )
        ) );   
    }else if(!empty($tag->get_option( 'bestselling' )[0]) && empty($tag->get_option( 'tags' )[0]) && empty($tag->get_option( 'category' )[0]) && empty($tag->get_option( 'featured' )[0])) {
        $woo_product_posts = get_posts( array(
        'post_type' => 'product',
        'post_status' => 'publish',
        'numberposts' => -1,
